...
```
We can then see that a relationship does exist with a p-value < 0.01 when we do not consider other factors. Vehicles in our sample with a manual transmission got on average `r summary(model1)$coefficients[2]` more miles per gallon. We must see if other confounders could also explain the relationship.
...
In this model with the vehicle weight added we see a high p-value for the manual transmission term. This would indicate that multicollinearity exists. Vehicle weight is correlated to both the transmission type and fuel efficiency. 
